...
# Remove singletons
ps_rare_filt = filter_taxa(ps_rare, function (x) {sum(x) > 1}, prune=TRUE)

# Find out the most relatively abundant 18S groups at order level (major groups >2% abundance)
order_18S <- tax_glom(ps_rare_filt, taxrank = "Order")
...
pvalue= pvalue[-c(1:10), ]
setEPS()
postscript("corrplot_18S.eps") # Set up .eps file of correlation plot to be exported
corrplot(coeff, p.mat = pvalue, outline = TRUE, type="full", insig="blank", sig.level =0.05, pch.cex = .9, tl.col = "black", tl.srt = 45) # Plot correlation plot 
dev.off() 

# Create UniFrac distance matrix for clustering analysis and dbRDA
ps_rel  <- transform_sample_counts(ps_rare_filt, function(x)x / sum(x))  # Normalize the data
...
pdf("dendrogram_18S.pdf")
plot(spe.ward, hang = -1, cex = 0.6)
rect.hclust(spe.ward, k=2)
dev.off()
...
ggsave(filename = "18S_dbRDA_1214.eps", plot = last_plot(), device = "eps", path = NULL, scale = 1, width = 7, height = 5, dpi = 600)

# Estimate Shannon diversity and richness using the rarefied phyloseq object with singletons (ps_rare) and compare between clusters
cluster = sample_data(ps_rare)$Cluster # Define variable from metadata (clusters)
alpha_div <- estimate_richness(ps_rare, measures=c("Observed", "Shannon")) # Estimate richness and diversity
alpha_div = cbind(alpha_div, cluster)
shapiro.test(alpha_div$Observed) # Shapiro Wilks normality test
...
norm_meta <-apply(meta_comp_new[,4:13], 2, function(x) shapiro.test(x)$p.value) # Perform Shapiro Wilks test on all factors of interest (p-value > 0.05 use t-test; p-value < 0.05 use Wilcoxon test)
...
# Remove singletons 
ps_16S_rare_filt = filter_taxa(ps_16S_rare, function (x) {sum(x) > 1}, prune=TRUE) # Remove singletons

# Find out the most relatively abundant 16S groups (at order level)
order_16S <- tax_glom(ps_16S_rare_filt, taxrank = "Order")
...
# Temporal abundance plots for top bacteria groups
...
pvalue= pvalue[-c(1:10), ]
setEPS()
postscript("corrplot_16S.eps") # Set up .eps file of 16S correlation plot to be exported
corrplot(coeff, p.mat = pvalue, outline = TRUE, type="full", insig="blank", sig.level =0.05, pch.cex = .9, tl.col = "black", tl.srt = 45) # Plot correlation plot 
dev.off() 

# Create UniFrac distance matrix for clustering analysis and dbRDA
ps_16S_rel  <- transform_sample_counts(ps_16S_rare_filt, function(x) x / sum(x)) # Normalize the data
...
p + geom_point(aes(fill=Month, shape=Cluster),size = 5, colour = "black") + scale_fill_viridis(discrete=TRUE, option="plasma", direction = -1)+
  ...
  geom_segment(mapping = arrow_map, size = 0.5, data = arrowdf, color = "black", arrow = arrowhead) + 
  geom_text(mapping = label_map, size = 4, color="black", data = arrowdf, show.legend = FALSE)
ggsave(filename = "16S_dbRDA_1214.eps", plot = last_plot(), device = "eps", path = NULL, scale = 1, width = 7, height = 5, dpi = 600)

# Estimate 16S Shannon diversity and richness and compare between clusters
cluster = sample_data(ps_16S_rare)$Cluster # Define variable from metadata (clusters)
alpha_div_16S <- estimate_richness(ps_16S_rare, measures=c("Observed", "Shannon")) # Estimate richness and diversity
alpha_div_16S = cbind(alpha_div_16S, cluster)
shapiro.test(alpha_div_16S$Observed) # Shapiro Wilks normality test
...
Y <- tax_table(ps)
Z <- sample_data(ps)
ps_network = merge_phyloseq(X,Y,Z) # Merge the phyloseq object
...
# Trim sets to include top 150 bacteria or protist ASV - improve sparsity and avoid ambiguous relationships
...
write_csv(Clus1_filt, "Skio-Clus1-network_08282021.csv")
taxALL_Clus1 <- tax_table(all_Clus1)
taxALL_Clus1 <- as.data.frame(taxALL_Clus1, check.names=FALSE)
write.table(taxALL_Clus1,"Skio-Clus1-TAX.txt",sep="\t", quote=FALSE)
...
write_csv(warm_filt, "Skio-Clus2-network_08282021.csv")
taxALL_Clus2 <- tax_table(all_Clus2)
taxALL_Clus2 <- as.data.frame(taxALL_Clus2, check.names=FALSE)
write.table(taxALL_Clus2,"Skio-Clus2-TAX.txt",sep="\t", quote=FALSE)

# Extract ASV-ASV pairings for Cluster 1 and 2 networks - Table S3
...
write.csv(newtab_final_Clus1, "Table_S3_Clus1.csv",row.names=F)
...
write.csv(newtab_final_Clus2, "Table_S3_Clus2.csv", row.names=F)
...
# Simplify kingdom level edge columns to indicate 16S, 18S, or a mixed (cross domain) relationship - new column "Type" will be created
newtab_merge[(newtab_merge$Edge2)=="Bacteria-Bacteria","Type"]<-"16S"
newtab_merge[(newtab_merge$Edge2)=="Archaea-Bacteria","Type"]<-"16S"
newtab_merge[(newtab_merge$Edge2)=="Bacteria-Archaea","Type"]<-"16S"
newtab_merge[(newtab_merge$Edge2)=="Eukaryota-Eukaryota","Type"]<-"18S"
newtab_merge[(newtab_merge$Edge2)=="Bacteria-Eukaryota","Type"]<-"Mixed"
newtab_merge[(newtab_merge$Edge2)=="Eukaryota-Bacteria","Type"]<-"Mixed"

# Number of kingdom level edges in Cluster 1 vs. 2 networks
...
newtab_merge_kingdom$Kingdom.x <- replace(as.character(newtab_merge_kingdom$Kingdom.x), newtab_merge_kingdom$Kingdom.x== "Archaea", "Bacteria") # Rename the few Archaea as Bacteria
newtab_merge_kingdom$Edge = paste(newtab_merge_kingdom$Kingdom.x, newtab_merge_kingdom$Kingdom.y,sep="-") # Create an edge with both kingdoms in the same cell
merge_kingdom_final = cbind(newtab_merge_kingdom,cluster, sign) # Merge pairings, cluster, and sign of the relationship
...
df = df[with(df, order(-Freq)), ] # Order relationships based on prevalance 

# Subset to the top ten 16S-18S relationships
mixed <- df[ df[["Type"]] == "Mixed" , ]
mixed[mixed == 0] <- NA
...
# Plot # of edges per node (degree) for Cluster 1/2 networks - compare at kingdom level
...
network_properties$Kingdom <- replace(as.character(network_properties$Kingdom), network_properties$Kingdom== "Archaea", "Bacteria")
...
# Repeat degree comparisons for major bacteria and protist groups
...
# Plot closeness centrality for Cluster 1/2 networks - compare at kingdom level
...
# Repeat centrality comparisons for major bacteria and protist groups
